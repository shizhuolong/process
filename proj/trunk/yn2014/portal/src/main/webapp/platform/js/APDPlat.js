...
GridBaseModel = function() {
    return {
        ...
        exportData: function(){
            parent.Ext.Ajax.request({
                ...
                success:function(response, opts){
                    ...
                    window.location.href= "download!download.action?filePath="+encodeURI(encodeURI(path))+"&fileName="+encodeURI(encodeURI(fn));
        ...
        //顶部工具条
        getToolbar: function(commands,tips,callbacks,securityNamespace){
            //工具栏组件
            var toolbar = new Ext.Toolbar();  
            this.addCommands(toolbar,true,commands,tips,callbacks,securityNamespace);
            return toolbar;
        },
        ...
        addCommands: function(obj,button,commands,tips,callbacks,securityNamespace){
            ...
            for(var i=0;i<commands.length;i++){
                ...
                if(null != securityNamespace) {
                	grantResult = parent.isGranted(securityNamespace,action,command);
                }else {
                	grantResult = parent.isGranted(namespace,action,command);
                }
                if(button){
                    if(command && grantResult){  
        ...
        getGrid: function(contextPath,namespace,action,pageSize, fields, columns, commands,tips,callbacks,securityNamespace){            
            ...
            this.toolbar=this.getToolbar(commands,tips,callbacks,securityNamespace);
        ...
        show : function(contextPath,namespace,action,pageSize, fields, columns, commands,tips,callbacks,securityNamespace){
            this.grid=this.getGrid(contextPath,namespace,action,pageSize, fields, columns, commands,tips,callbacks,securityNamespace);
            this.getViewport();    
        }
    };
} ();
...
AdvancedSearchBaseModel = function() {
    return {
        ...
        silentSearch : function(data,alias,criteria){
            var queryString="";
            var propertyCriteria = "";
            for(var i=0;i<criteria.length;i++){
                if(criteria[i]!=""){
                	propertyCriteria+=criteria[i];
        ...
        search: function(data,alias,criteria){
            if(this.silentSearch(data,alias,criteria)){
