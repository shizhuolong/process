	...
	$("#renewBtn").click(function(){
		...
		$('input[name="selected"]:checked').each(function(){  
			...
			if(id_array.length>=2){
				var url="/portal/portal/contract/jsp/channel_batch_renew.jsp?id="+idstr;
				//window.parent.openWindow("批量续签",'funMenu',url);
				window.location.href="/portal/portal/contract/jsp/channel_batch_renew.jsp?id="+idstr;
			}else{
				art.dialog({
					title: '提示',
		   		    content: '选择的数量小于2',
...
function search(pageNumber) {
	...
	$.ajax({
		...
		data:{
           ...
           "resultMap.hqChanCode":hqChanCode,
           "resultMap.startDate":$("#startDate").val(),
           "resultMap.endDate":$("#endDate").val()
	   	}, 
