...
function downDevData(){
	...
	if(devStartDate==devEndDate){
		title=[
				["云南联通自营厅新增发展日通报","","","","","","","","","","","","",""],
				["单位：户","本月截止：",day+" 日","-","","","","","","","","","",""],
				["分公司","厅数","日发展","","月累计","","发展累计","单厅","累计环比(%)","","","定比上季度月均","定比排名","年度日累计发展时序完成率"],
...
function getDevSql(){
	...
	if(devStartDate==devEndDate){
		...
		"            LEFT JOIN (SELECT NVL(GROUP_ID_1_NAME, '全省') GROUP_ID_1_NAME,                                                                                       "+
		"                            ROUND((NVL(SUM(THIS_YW_DEV1), 0) + NVL(SUM(THIS_NET_DEV1), 0))/3,3) LAST_SEASON_DEV                                                               "+
		"                       FROM PMRT.TB_MRT_BUS_ZY_REPORT_DETAIL                                                                                                     "+
		"      WHERE DEAL_DATE IN (SELECT * FROM TABLE (PMRT.LAST_QUAR_DEAL1("+devStartDate+")))"               +
		                          where+
