var editor=null;
		...
		$(function(){
			...
			$("#cancelBtn").click(function(){
				window.parent.window.art.dialog({ id: 'bullEditDialog' }).close();
			});
			$("#addBtn").click(function(){
				...
				$("INPUT[name='file']").each(function(){
					$(this).next().val($(this).val().substring($(this).val().lastIndexOf("\\")+1));
				});
				$("#bullForm").submit();
			});
		});
		...
		KindEditor.ready(function(K) {
			editor=K.create('textarea[name="content"]', {
				autoHeightMode : true,
				uploadJson : $("#ctx").val()+'/bullManagement/bullManager_uploadfile.action',
				filePostName:'image',
			...
			if(null!=id&&id.length>0){
				...
				if(isTop=='1'){
					...
					$("#isTop").val('0');
					$("#isTop").find("option[value='0']").attr("selected",true);
				}
				if(isShow=='1'){
					$("#isShow").val('1');
					...
					$("#isShow").val('0');
					$("#isShow").find("option[value='0']").attr("selected",true);
				}
				if(isManage=='1'){
					$("#isManage").val('1');
					$("#isManage").find("option[value='1']").attr("selected",true);
				}else{
					$("#isManage").val('0');
					$("#isManage").find("option[value='0']").attr("selected",true);
				}
				editor.html(content);
				...
				if(attachmentsNames.length>0){
					...
					var oa="";
					for(var j=0;j<i;j++){
						oa+="<a target='_blank' class='attachFlag' myhref='"+attUrls[j]+"' href='"+$("#ctx").val()+"/bullManagement/bullManager_downfile.action?downUrl="+attUrls[j]+"&downName="+encodeURI(encodeURI(atts[j]))+"' >"+atts[j]+"</a>&nbsp;<a href='javascript:void(0);' onclick='deleteOldAttach(this)' >删除</a><br/>";
					}
