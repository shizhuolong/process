var nowData = [];
var title=[["账期","地市","营服中心","配置平台站址编码","业务确认单编号","站点编码","需求确认单编号","站址名称","站点名称","经度","纬度","产品种类","产品场景","建筑面积/隧道长度（平方米/公里）","系统数量","当前共享客户总数","后备电池时长（小时）","电力保障服务费模式","包干电费（元/年）不含税","其他费用（元/年）不含税","其它费用说明","基准价格（元/年）不含税","维护费（元/年）不含税","产品单元数","场地费（元/年）不含税","共享折扣","服务起始日期","服务结束日期","产品服务费合计（元/年）不含税","产品服务费合计（元/年）含税"]];
...
function search(pageNumber) {
	...
	var sql="SELECT "+field.join(",")+" FROM PMRT.TAB_MRT_IRON_PRO_MON_TEMP WHERE DEAL_DATE='"+time+"' AND GROUP_ID_1='"+regionCode+"'";
	var csql = sql;
	...
	if(totalCount){
		if(confirm("确认导入？")){
			...
			$.ajax({
				...
				async:true,
				cache:false,
				url:paths+"/channelManagement/ironPro_importToResult.action",
				data:{
		           "time":time,
		           "userId":userId,
