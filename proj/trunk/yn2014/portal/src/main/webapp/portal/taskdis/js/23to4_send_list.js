var nowData = [];
var curPage=0;
var title=[["操作","分配批次","接收人","营服名称","用户号码","用户编码","前三月平均缴费","终端类型","终端品牌","终端型号","发展渠道","渠道编码","入网时间","套餐ID","套餐名称"]];
var field=["STATE","BATCH_NO","RECEIVER","UNIT_NAME","DEVICE_NUMBER","SUBSCRIPTION_ID","LAST_3_PAY","TERMINAL_TPYE","TERMINAL_DOM","TERMINAL_MOD","DEV_CHNL","DEV_CHNL_NAME","INNET_DATE","PRODUCT_ID","PRODUCT_NAME"];
var report = null;
...
function search(pageNumber) {
	...
	sql+="        ,(SELECT REALNAME FROM PODS.VIEW_23TO4_USER U WHERE U.ID=D.USERID)     RECEIVER ";
	sql+="        ,T.UNIT_NAME                                   ";
	sql+="        ,T.DEVICE_NUMBER                               ";
	...
	if(receiver!=''){
		sql+=" and (SELECT REALNAME FROM PODS.VIEW_23TO4_USER U WHERE U.ID=D.USERID) ='"+receiver+"'                          ";
	}
...
function exportAll(){
	...
	var title=[["分配批次","接收人","营服名称","用户号码","用户编码","前三月平均缴费","终端类型","终端品牌","终端型号","发展渠道","渠道编码","入网时间","套餐ID","套餐名称"]];
	var field=["BATCH_NO","RECEIVER","UNIT_NAME","DEVICE_NUMBER","SUBSCRIPTION_ID","LAST_3_PAY","TERMINAL_TPYE","TERMINAL_DOM","TERMINAL_MOD","DEV_CHNL","DEV_CHNL_NAME","INNET_DATE","PRODUCT_ID","PRODUCT_NAME"];
