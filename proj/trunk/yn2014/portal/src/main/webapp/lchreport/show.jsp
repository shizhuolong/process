	...
	<div id="lchContent" style="position: absolute;width: 100%; height: 100%;margin:0;padding:0;">
		...
		<div class="cpBody">
			<div id="editTable" style="position: absolute; overflow: hidden;">
				<table width="100%" height="100%" cellspacing=0 cellpadding=0 style="border-top:none;">
					<tr>
						<td  class="title" style="border-right: solid #e7d4b3 1px;border-bottom: solid #e7d4b3 1px;border-top: none;">
						</td>
...
<script>
		//导出数据
		function downAll(){
			var url=$("#ctx").val()+"/lchreport?action=downreport";
			var form = $("#condition");  
			form.attr('target','');  
			...
			if(!form.find("INPUT[name='report']").length){
				...
				//删除report多余生成的contitions属性
				delete report.condition.conditions;
		...
		function ajustHead($t){
			...
			$t.height($t.height()-$(".condition").height());
			$top.width($body.width());
			$top.find("TD[colspan='1']").each(function(i){
				var $cx=$body.find("TD[x='"+i+"'][colspan='1']:visible");
				var $cxt=$(this);
				if($cx.width()>$cxt.width()){
					$cxt.width($cx.width());
				}else{
					$cx.width($cxt.width());
				}
			...
			$left.find("TD[rowspan='1']").each(function(i){
				var $cx=$body.find("TD[y='"+i+"'][rowspan='1']:visible");
				var $cxt=$(this);
				if($cx.height()>$cxt.height()){
					$cxt.height($cx.height());
				}else{
					$cx.height($cxt.height());
				}
			});
			////////////////////////
		...
		function createTableTitle(headData){
				...
				for(var y=0;y<headData.length;y++){
					...
					for(var x=0;x<headData[0].length;x++){
						...
						h+='<td x="'+x+'" y="'+y+'" class="title" colspan="'+dx+'" rowspan="'+dy+'" style="word-break:break-all;" >'+headData[y][x]+'</td>';
		...
		function getPage(num){
			var url=$("#ctx").val()+"/lchreport?action=listreport";
