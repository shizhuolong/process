		...
		getSubRowsCallBack:function($tr){
			...
			if($tr){
				...
				if(orgLevel==1){//省
					...
					provinceSql=' union all select t.group_id_0 ROW_ID,\'全省合计\' ROW_NAME,'+sumSql+' from PMRT.TAB_MRT_TARGET_ORIGINAL_MON t ';
			...
			var sql='select '+preField+','+sumSql+' from PMRT.TAB_MRT_TARGET_ORIGINAL_MON t ';
...
function getdownField(where){
 ...
 var sql="SELECT T1.GROUP_ID_1_NAME,                                                                 " 
	...
	+"                                    END,                                                       "
	+"                                    2) AVG_FLOW_4G                                             "
	+"          FROM PMRT.TAB_MRT_TARGET_ORIGINAL_MON T                                                    "
	+where
