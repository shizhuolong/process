...
function search(pageNumber) {
	...
	var sql="select DEAL_DATE,GROUP_ID_1,CREATOR,CREATETIME,HR_NO,USER_NAME,POST_LEVEL,SALARY_LEVEL,POST_SALARY,GENERAL_SUBS,DIFFICULT_AREAS,MERIT_PAY_1,MERIT_PAY_2,OTHER_PAY_1,OTHER_PAY_2,OVERTIME_PAY,FESTIVITY_PAY,CHINA_ONE_PAY,MULTI_WY,MULTI_WC,MULTI_JT,MULTI_TX,MULTI_DSR,OTHER2,SALARY_PAY_TOTAL,PROVIDE_AGE,TREATMENT,UNEMPLOYE,HOUSING,SUPPLEMENTARY,INCOME_TAX,OTHER_COST_1,OTHER_COST_1_ITEM,DEDUCTED_TOTAL,FACT_TOTAL from PTEMP.TB_TMP_JCDY_HR_SALARY_TEMP WHERE DEAL_DATE='"+time+"' AND CREATOR='"+userId+"'";
	var csql = sql;
	...
	if(totalCount){
		if(confirm("确认导入？")){
			...
			$.ajax({
				...
				async:true,
				cache:false,
				url:paths+"/devIncome/hrUploadOld_confirmTax.action",
				data:{
		           "time":time,
		           "userId":userId,
			   	...
			   	success:function(data){
			   		if(data&&data.ok){
			   			alert("已经成功入库");
			   			window.location.href=paths+"/report/devIncome/jsp/tmp_jcdy_hr_old_salary.jsp";
			   		}else{
			   			alert("入库失败,请重试");
			   			$("#confirmBtn").show();
			   		}
		...
		alert("没有要入库的数据,请先导入");
		window.location.href=$("#ctx").val()+"/report/devIncome/jsp/tmp_jcdy_hr_old_salary.jsp";
	}
  }
