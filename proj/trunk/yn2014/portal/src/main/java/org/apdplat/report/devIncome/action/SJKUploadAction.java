...
@Controller
@Scope("prototype")
public class SJKUploadAction extends BaseAction{
	...
	public void confirmKpi(){
		...
		try{
			...
			String delRepeat="DELETE PMRT.TAB_MRT_JCDY_KPI_QJ_MON T                          "+
					         ...
					         "AND T.KPI_NAME IN (SELECT KPI_NAME FROM PMRT.TAB_MRT_JCDY_KPI_QJ_MON_TEMP"+
					         " WHERE DEAL_DATE='"+time+"' AND GROUP_ID_1='"+regionCode+"')"+
					         "AND T.HR_ID IN (SELECT HR_ID FROM PMRT.TAB_MRT_JCDY_KPI_QJ_MON_TEMP WHERE DEAL_DATE='"+time+"' AND GROUP_ID_1='"+regionCode+"' ) AND IS_KPI=2";
			...
			String importToResult="INSERT INTO PMRT.TAB_MRT_JCDY_KPI_QJ_MON                                                                      "+
					             "SELECT T1.DEAL_DATE,T2.GROUP_ID_1,T3.GROUP_ID_1_NAME,T2.UNIT_ID,T2.UNIT_NAME,T1.HR_ID,T0.USER_CODE,T1.KPI_NAME,"+
				                	"'',T1.KPI_SCORE,86000,T1.KPI_WEIGHT,T4.REALNAME,T1.LOGIN_NAME,2 IS_KPI                                      "+
					              "FROM  PMRT.TAB_MRT_JCDY_KPI_QJ_MON_TEMP T1                                                                    "+
	...
	public String importKpi(){
		...
		String resultTableName="PMRT.TAB_MRT_JCDY_KPI_QJ_MON_TEMP";
		if(uploadFile==null){
			err.add("上传文件为空");
		}else{
			try{
				...
				if(sheetNum>0){
					...
					for(int y=start;y<=end;y++){
						String sql="INSERT INTO PMRT.TAB_MRT_JCDY_KPI_QJ_MON_TEMP(DEAL_DATE,GROUP_ID_1,LOGIN_NAME,UNIT_ID,UNIT_NAME,USER_NAME,HR_ID,KPI_NAME,KPI_SCORE,KPI_WEIGHT)";
					...
					String trim="update PMRT.TAB_MRT_JCDY_KPI_QJ_MON_TEMP T SET T.UNIT_ID=TRIM(T.UNIT_ID),T.HR_ID=TRIM(HR_ID),T.KPI_SCORE=TRIM(T.KPI_SCORE),T.KPI_WEIGHT=TRIM(T.KPI_WEIGHT)";
					System.out.println("trim开始-----------------------");
					SpringManager.getUpdateDao().update(trim);
					String isUnitIdError="SELECT T1.UNIT_ID,T1.UNIT_NAME FROM PMRT.TAB_MRT_JCDY_KPI_QJ_MON_TEMP T1                                                                           "+
							             "WHERE NOT EXISTS (SELECT 1 FROM PCDE.TAB_CDE_GROUP_CODE t2 where T1.UNIT_ID=T2.UNIT_ID AND T2.IS_FIVE_CLASS=5 AND T2.GROUP_ID_1='"+regionCode+"') "+
					...
					String isHrIdError="SELECT T1.HR_ID,T1.USER_NAME FROM PMRT.TAB_MRT_JCDY_KPI_QJ_MON_TEMP T1                       "+
					...
					String isCrossUnit="SELECT * FROM PMRT.TAB_MRT_JCDY_KPI_QJ_MON_TEMP T1                                                                  "+
					...
					String kpiWeight = "SELECT * FROM (                                                                  "
							...
							+ "SELECT T1.HR_ID,REPLACE(T1.KPI_WEIGHT,'%') KPI_WEIGHT FROM PMRT.TAB_MRT_JCDY_KPI_QJ_MON_TEMP T1 "
							+ "WHERE T1.GROUP_ID_1='"
							+ regionCode
							+ "' AND T1.DEAL_DATE='"
							+ time
							...
							+"AND("
							+ " T2.HR_ID NOT IN "
							+ "(SELECT HR_ID FROM PMRT.TAB_MRT_JCDY_KPI_QJ_MON_TEMP WHERE DEAL_DATE='"
							+ time
							+ "' AND GROUP_ID_1='"
							+ regionCode
							+ "') OR T2.KPI_NAME NOT IN(SELECT KPI_NAME FROM PMRT.TAB_MRT_JCDY_KPI_QJ_MON_TEMP WHERE DEAL_DATE = '"+time+"' AND GROUP_ID_1 = '"+regionCode+"')))"
							+ " GROUP BY HR_ID ) WHERE KPI_WEIGHT>100";
					...
					if(wResult!=null&&!wResult.isEmpty()){
						for(Map<String,String> m:wResult){
							err.add("HR编码:"+m.get("hr_id")+"的权重超过100%,请检查!");
						}
						Struts2Utils.getRequest().setAttribute("err", err);
						return "error";
					}
