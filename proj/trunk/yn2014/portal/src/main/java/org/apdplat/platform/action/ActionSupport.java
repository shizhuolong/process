...
public abstract class ActionSupport extends DataPrivilegeControl{
    ...
    public Long[] getIds() {
        if (ids != null && ids.contains("-") && ids.contains(",")) {
            ...
            for (String info : idInfo) {
                if (info.contains("-")) {
                    ...
                    long start = Long.parseLong(inner[0]);
                    long end = Long.parseLong(inner[1]);
                    if (start > end) {
                        long temp = start;
                        start = end;
                        end = temp;
                    ...
                    result.add(Long.parseLong(info));
                }
            }
        ...
        if (ids != null && ids.contains("-")) {
            String[] idInfo = ids.split("-");
            long start = Long.parseLong(idInfo[0]);
            long end = Long.parseLong(idInfo[1]);
            if (start > end) {
                long temp = start;
                start = end;
                end = temp;
        ...
        if (ids != null && ids.contains(",")) {
            ...
            for (int i = 0; i < idInfo.length; i++) {
                result.add(Long.parseLong(idInfo[i]));
            }
            return result.toArray(new Long[result.size()]);
        }
        if (ids != null) {
