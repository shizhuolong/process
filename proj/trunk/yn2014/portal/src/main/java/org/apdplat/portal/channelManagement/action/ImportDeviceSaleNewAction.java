...
@Scope("prototype")
@Results({
	@Result(name="success", location="/portal/channelManagement/jsp/import_deviceSale_list_new.jsp"),
	@Result(name="error", location="/portal/channelManagement/jsp/import_deviceSale_new.jsp")
})
public class ImportDeviceSaleNewAction extends BaseAction {
	...
	public String importToTemp() {
		...
		String field="INPUT_TIME,INPUT_USER,MODEL_TYPE,DEAL_DATE,HQ_CHAN_CODE,SALE_NUM";
		if (uploadFile == null) {
			err.add("上传文件为空！");
		} else {
			...
			try {
				...
				if (sheetNum > 0) {
					...
					Row row;
					String sql = "INSERT INTO "+resultTableName+"("+field+") values(sysdate,'"+username+"',1,?,?,?)";
					...
					for (int y = start; y <= end; y++) {
						...
						for (int i = cstart; i < cend; i++) {
						    if(i==4){
						        String date = getCellValue(row.getCell(i));
						        pre.setString(1,date);
						    }
						    if(i==17){
                                pre.setString(2,getCellValue(row.getCell(i)));
                            }
						    if(i==25){
                                pre.setString(3,getCellValue(row.getCell(i)));
                            }
	...
	public void downfile() {
		File f=new File(ServletActionContext.getServletContext().getRealPath("/portal/channelManagement/down/import_deviceSale_new.xls"));
