...
@Controller
@Scope("prototype")
public class TerminalSalesAction extends BaseAction{
	...
	public void importToResult(){
		...
		try{
			...
			String dsql="DELETE FROM PMRT.TB_TERMINAL_SALES_DAY WHERE DEAL_DATE IN (SELECT DEAL_DATE FROM PMRT.TB_TERMINAL_SALES_DAY"+ 
					" WHERE DEAL_DATE IN(SELECT DEAL_DATE FROM PMRT.TB_TERMINAL_SALES_DAY_TEMP WHERE IN_USER_NAME='"+username+"')   "+
					") AND IN_USER_NAME='"+username+"'                                                                              ";
			SpringManager.getUpdateDao().update(dsql);
			String sql="INSERT INTO PMRT.TB_TERMINAL_SALES_DAY SELECT * FROM PMRT.TB_TERMINAL_SALES_DAY_TEMP WHERE IN_USER_NAME='"+username+"'";
	...
	public void downfile() {
		File f=new File(this.request.getRealPath("/portal/channelManagement/down/import_terminal_sales.xls"));
		...
		try{
			...
			resp.addHeader("content-disposition", "attachment;filename=import_terminal_sales.xls");
	...
	public String importToTemp(){
		...
		String resultTableName="PMRT.TB_TERMINAL_SALES_DAY_TEMP";
		if(uploadFile==null){
			err.add("上传文件为空");
		}else{
			try{
