...
@Scope("prototype")
@Results({
	@Result(name="success", location="/portal/channelManagement/jsp/import_rent_list_new.jsp"),
	@Result(name="error", location="/portal/channelManagement/jsp/import_rent_new.jsp")
})
public class ImportRentNewAction extends BaseAction {
	...
	public void importToResult() throws SQLException {
		String delRepeat = "DELETE PMRT.TAB_MRT_RENT_MON WHERE DEAL_DATE='"+ time+"'";
		SpringManager.getUpdateDao().update(delRepeat);
		String importToResult = "INSERT INTO PMRT.TAB_MRT_RENT_MON SELECT * FROM PMRT.TAB_MRT_RENT_MON_TEMP WHERE DEAL_DATE='"+time+"'";
	...
	public void downfile() {
		File f=new File(this.request.getRealPath("/portal/channelManagement/down/import_rent_new.xls"));
		...
		try{
			...
			resp.addHeader("content-disposition", "attachment;filename=import_rent_new.xls");
