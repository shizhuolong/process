...
@Scope("prototype")
@Results({
	@Result(name="success", location="/portal/channelManagement/jsp/import_sub_list.jsp"),
	@Result(name="error", location="/portal/channelManagement/jsp/import_sub.jsp")
})
public class ImportSubAction extends BaseAction {
	...
	public void importToResult() {
		...
		try {
			...
			String delRepeat = "DELETE PMRT.TAB_MRT_SUBS_MAINTAIN_MON WHERE DEAL_DATE='"	+ time+ "' AND GROUP_ID_1="+ regionCode;

			SpringManager.getUpdateDao().update(delRepeat);
			String importToResult = "INSERT INTO PMRT.TAB_MRT_SUBS_MAINTAIN_MON SELECT * FROM PMRT.TAB_MRT_SUBS_MAINTAIN_MON_TEMP WHERE DEAL_DATE='"+time+"' AND GROUP_ID_1="+ regionCode;
			...
			try {
				...
				if (sheetNum > 0) {
					...
					for (int y = start; y <= end; y++) {
						String sql = "INSERT INTO PMRT.TAB_MRT_SUBS_MAINTAIN_MON_TEMP";
	...
	public void downfile() {
		File f=new File(this.request.getRealPath("/portal/channelManagement/down/import_sub.xls"));
		...
		try{
			...
			resp.addHeader("content-disposition", "attachment;filename=import_sub.xls");
