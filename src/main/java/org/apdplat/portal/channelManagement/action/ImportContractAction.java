...
@Scope("prototype")
@Results({
	@Result(name="success", location="/portal/channelManagement/jsp/import_contract_list.jsp"),
	@Result(name="error", location="/portal/channelManagement/jsp/import_contract.jsp")
})
public class ImportContractAction extends BaseAction {
	...
	public String importToTemp() {
		...
		String field="DEAL_DATE,GROUP_ID_1,UNIT_ID,UNIT_NAME,USERNAME,CONTRACT_NO,CONTRACT_NAME,CELL_NAME,YT_TYPE,BEGIN_FEE,THIS_MON_YT,THIS_MON_YT_STA,PAY_FEE,PRICE,FAX,END_FEE,GROUP_ID_1_NAME,ACPEFIX,CELL_ID,IS_COM,IS_WY_FEE";
		if (uploadFile == null) {
			err.add("上传文件为空！");
		} else {
			...
			try {
				...
				if (sheetNum > 0) {
					...
					int start = sheet.getFirstRowNum() +4 ;// 去前4行标题
					int end = sheet.getLastRowNum();
					Row row;
					String sql = "INSERT INTO PMRT.TAB_MRT_CONTRACT_ALL_MON_TEMP("+field+") values('"+time+"','','','','"+username+"',?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
	...
	public void downfile() {
		File f=new File(this.request.getRealPath("/portal/channelManagement/down/import_contract.xls"));
		...
		try{
			...
			resp.addHeader("content-disposition", "attachment;filename=import_contract.xls");
