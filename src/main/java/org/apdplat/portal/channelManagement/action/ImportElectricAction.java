...
public class ImportElectricAction extends BaseAction {
	...
	public void importToResult() {
		...
		try {
			...
			String delRepeat = "DELETE PMRT.TAB_MRT_ELECTRIC_CHARGE_MON  WHERE DEAL_DATE='"	+ time+ "' AND GROUP_ID_1="+ regionCode;

			SpringManager.getUpdateDao().update(delRepeat);
			String importToResult = "INSERT INTO PMRT.TAB_MRT_ELECTRIC_CHARGE_MON  SELECT * FROM PMRT.TAB_ELECTRIC_CHARGE_MON_TEMP WHERE DEAL_DATE='"+time+"' AND GROUP_ID_1="+ regionCode;
			...
			try {
				...
				if (sheetNum > 0) {
					...
					for (int y = start; y <= end; y++) {
						String sql = "INSERT INTO PMRT.TAB_ELECTRIC_CHARGE_MON_TEMP";
