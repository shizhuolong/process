...
public class BusinessHallPersonAction extends BaseAction {
	...
	public void listTreeNode() {
		...
		List<Map<String, Object>> list = businessHallPersonService
				.listTreeNode(params);
	...
	public void update() {
		...
		String user_code = request.getParameter("user_code").trim();
		...
		// deal_date 帐期
		String deal_date = request.getParameter("deal_date").trim();
		m.put("deal_date", deal_date);
		...
		//logger.info("f_hr_id=====================|"+f_hr_id+"|===========");
		try {
			businessHallPersonService.updateBussiness(m);
