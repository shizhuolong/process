...
public class ImportElectricMaintainAction extends BaseAction {
	...
	public void importToResult() {
		...
		try {
			...
			String delRepeat = "DELETE PMRT.TAB_MRT_ELECTRIC_CHARGE_MON1 WHERE DEAL_DATE='"	+ time+ "' AND USERNAME='"+ username+"'";

			SpringManager.getUpdateDao().update(delRepeat);
			String importToResult = "INSERT INTO PMRT.TAB_MRT_ELECTRIC_CHARGE_MON1 SELECT * FROM PMRT.TAB_ELECTRIC_CHARGE_MON1_TEMP WHERE DEAL_DATE='"+time+"' AND USERNAME='"+ username+"'";
