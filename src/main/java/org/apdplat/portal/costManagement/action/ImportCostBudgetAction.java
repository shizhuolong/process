...
@Scope("prototype")
@Results({
	@Result(name="index", location="/portal/costManagement/jsp/import_cost_budget_list.jsp")
})
public class ImportCostBudgetAction extends BaseAction{
		...
		if(myFile != null && myFile.length > 0) {
			...
			if("".equals(resultMsg)) {
				...
				try {
					...
					List<Map<String, Object>> datas = importCostBudgetService.queryNotExistsUnit(params);
					if(datas.size() > 0) {
						resultMsg = "成本中心名称【";
						for(int i=0; i<datas.size(); i++) {
							Map<String, Object> m = datas.get(i);
							String unit_name = m.get("COST_CENTER_NAME").toString();
							resultMsg += unit_name+"、";
						}
					...
					if("".equals(resultMsg)) {
						...
						List<Map<String, Object>> existsDatas = importCostBudgetService.queryExistsCostName(p);
						if(existsDatas.size() > 0) {
							for(int i=0; i<existsDatas.size(); i++) {
								Map<String, Object> exis = existsDatas.get(i);
								String deal_date = String.valueOf(exis.get("DEAL_DATE"));
								String cost_center_name = String.valueOf(exis.get("COST_CENTER_NAME"));
								resultMsg += deal_date+"帐期"+cost_center_name+"、";
							}
	...
	public void loadById() {
		...
		Map<String, String> params = new HashMap<String, String>();
		params.put("id", id);
		List<Map<String, Object>> list = importCostBudgetService.loadById(params);
