	...
	public void confirmKpi() {
		...
		try {
			...
			String delRepeat = "DELETE PMRT.TAB_MRT_JCDY_KPI_QJ_MON T                          "
					...
					+ regionCode
					+ "')"
					+ "AND   T.HR_ID IN (SELECT HR_ID FROM PCDE.IS_KPI_TEMP WHERE DEAL_DATE='"
					+ time
					+ "' AND GROUP_ID_1='"
					+ regionCode
					+ "' ) AND T.IS_KPI=1";
			...
			String importToResult = "INSERT INTO PMRT.TAB_MRT_JCDY_KPI_QJ_MON                                                              "
					...
					+ "'',T1.KPI_SCORE,86000,T1.KPI_WEIGHT,T4.REALNAME,T1.LOGIN_NAME,1 IS_KPI                                      "
					+ "FROM  PCDE.IS_KPI_TEMP T1                                                                    "
					+ "JOIN  PORTAL.TAB_PORTAL_QJ_PERSON T0                                                                           "
					+ "ON    (T1.HR_ID=T0.HR_ID AND T0.DEAL_DATE='"
					+ time
					+ "')                                                          "
			...
			try {
				...
				if (sheetNum > 0) {
					...
					for (int y = start; y <= end; y++) {
						...
						String values = " values('" + time + "','" + regionCode
								+ "','" + userId + "'";
						...
						if (cstart == 0 && cend == 7) {
							...
							int n = SpringManager.getUpdateDao().update(
									sql + values);
							if (n <= 0) {
					...
					String isUnitIdError = "SELECT T1.UNIT_ID,T1.UNIT_NAME FROM PCDE.IS_KPI_TEMP T1                                                                           "
							+ "WHERE NOT EXISTS (SELECT 1 FROM PCDE.TAB_CDE_GROUP_CODE t2 where T1.UNIT_ID=T2.UNIT_ID AND T2.IS_FIVE_CLASS<>5 AND T2.GROUP_ID_1='"
							+ regionCode
							+ "') "
							+ "AND T1.GROUP_ID_1='"
							+ regionCode + "' AND T1.DEAL_DATE='" + time + "'";
					List<Map<String, String>> uResult = SpringManager
							.getFindDao().find(isUnitIdError);
					...
					List<Map<String, String>> hResult = SpringManager
							.getFindDao().find(isHrIdError);
					...
					String isCrossUnit = "SELECT * FROM PCDE.IS_KPI_TEMP T1                                                                  "
							+ "JOIN (SELECT DISTINCT HR_ID,UNIT_ID FROM PORTAL.VIEW_U_PORTAL_PERSON WHERE DEAL_DATE='"
							+ time
							+ "') T2 "
					...
					List<Map<String, String>> cResult = SpringManager
							.getFindDao().find(isCrossUnit);
					...
					String kpiWeight = "SELECT * FROM (                                                                  "
							...
							+" AND("
							+ " T2.HR_ID NOT IN "
							+ "(SELECT HR_ID FROM PCDE.IS_KPI_TEMP WHERE DEAL_DATE='"
							+ time
							+ "' AND GROUP_ID_1='"
							+ regionCode
							+ "') OR T2.KPI_NAME NOT IN(SELECT KPI_NAME FROM PCDE.IS_KPI_TEMP WHERE DEAL_DATE = '"+time+"' AND GROUP_ID_1 = '"+regionCode+"')))"
							+ " GROUP BY HR_ID ) WHERE KPI_WEIGHT>10";

					List<Map<String, String>> wResult = SpringManager
							.getFindDao().find(kpiWeight);
