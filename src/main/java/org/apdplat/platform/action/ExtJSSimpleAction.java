...
public abstract class ExtJSSimpleAction<T extends Model> extends ExtJSActionSupport implements Action {
                ...
                if(property.getName().contains(".")){
                     ...
                     if(property.getName().contains(".id")){
                        ...
                        if(attr.length==2){
                            ...
                            T change=getService().retrieve((Class<T>)field.getType(), (Long)property.getValue());
    ...
    public String delete() {
        try{
            ...
            List<Long> deletedIds=getService().delete(modelClass, getIds());
    ...
    public void afterDelete(List<Long> deletedIds){
    ...
    public String export() {
    	this.setPage(service.query(modelClass, getPageCriteria(), buildPropertyCriteria(), super.buildOrderCriteria()));
    ...
    protected void afterSuccessCreateModel(T model) {
    	if(model!=null&&model instanceof Module){
    		getService().createMenuRole((Module)model);
    	}
    }
    /**
     * 模型【创建失败】后的回调方法
