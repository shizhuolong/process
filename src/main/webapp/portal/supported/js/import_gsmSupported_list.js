...
function search(pageNumber) {
	...
	$.ajax({
		...
		cache:false,
		async: false,
		url:$("#ctx").val()+"/supported/gsmSupported!list.action",
		data:{
...
function initTotalFee(){
	$.ajax({
		...
		url:$("#ctx").val()+"/supported/gsmSupported!queryTotalFee.action",
	   	success:function(data){
...
function del(obj){
	var bill_id=obj.attr("bill_id");
	if(confirm('确认刪除吗?')){
	  window.location.href=$("#ctx").val()+"/supported/gsmSupported!del.action?bill_id="+bill_id;
	  //search(0);
	}
}
...
function save(){
	...
	$("#bill_id").val(bill_id);
	var url = $("#ctx").val()+'/supported/gsmSupported!update.action';
...
function downExcelTemp() {
	location.href = $("#ctx").val()+"/supported/gsmSupported!downloadTemplate.action";
}
	...
	if(validate()) {
		art.dialog.confirm("您确定提交审批吗？",function(){
			...
			$("#taskForm").form("submit",{
				url:$("#ctx").val()+'/supported/gsmSupported!doSubmitTask.action',
				onSubmit:function(){
					jQuery.blockUI({
