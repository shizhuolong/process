$(function(){
	var maxDate=getMaxDate("PMRT.TB_MRT_USER_WX_SCHEME_DAY");
	if(maxDate!=null){
		$("#dealDate").val(maxDate);
	}
	var title=[["组织架构","营服属性","渠道状态","当日受理用户数","本月累计受理用户数"]];
	var field=["ROW_NAME","UNIT_TYPE","STATUS","SERVICE_NUM","SERVICE_NUM1"];
    ...
    var report=new LchReport({
		...
		getSubRowsCallBack:function($tr){
			...
			var where=" WHERE DEAL_DATE LIKE '%"+getYearMon(dealDate)+"%' AND DEAL_DATE<="+dealDate+"";
			if($tr){
				code=$tr.attr("row_id");
...
function downsAll() {
	...
	var showtext = '老用户维系活动受理日报-' + dealDate;
	var title=[["地市","营服名称","营服属性","渠道","渠道状态","当日受理用户数","本月累计受理用户数"]];
...
function getSumSql(){
	var dealDate=$("#dealDate").val();
	return " ,NVL(COUNT(CASE WHEN DEAL_DATE='"+dealDate+"'"+
	"                  THEN SUBSCRIPTION_ID             "+
	"                  END) ,0) SERVICE_NUM             "+
	"       ,NVL(COUNT(SUBSCRIPTION_ID),0)  SERVICE_NUM1"+        
	" FROM PMRT.TB_MRT_USER_WX_SCHEME_DAY                ";
}

function getYearMon(dealDate){
	return dealDate.substring(0,6);
}
