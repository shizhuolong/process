...
function search(pageNumber) {
	...
	var sql="SELECT "+field.join(",")+" FROM PMRT.TAB_MRT_IRON_STOCK_MON_TEMP WHERE DEAL_DATE='"+time+"' AND GROUP_ID_1='"+regionCode+"' AND HALL_TYPE='"+hallType+"'";
	var csql = sql;
	...
	if(totalCount){
		if(confirm("确认导入？")){
			...
			$.ajax({
				...
				async:true,
				cache:false,
				url:paths+"/channelManagement/ironStock_importToResult.action",
				data:{
		           "time":time,
		           "userId":userId,
