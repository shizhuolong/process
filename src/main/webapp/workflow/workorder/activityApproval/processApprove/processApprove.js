...
$(function(){
	...
	function loadData(){
		baseInfo.listType = $("#orderState").val();
...
var baseInfo={
	...
	//  $("#dialogid").dialog({title: '加载中...',modal:true,width:222,closeable:false});
		...
		$.ajax({
			...
			dataType:'json',
			cache:false,
			url:path+"/workflow/work-flow!qryProcessOrderList.action",
			data:{
		           "workOrderVo.queryListType":baseInfo.listType,
		           "workOrderVo.displayPid":baseInfo.workNo,
		           "workOrderVo.title":baseInfo.title,
		           "workOrderVo.createTime":baseInfo.createTime+" 00:00:00",
		           "workOrderVo.pageNo":pageNumber,
		           "workOrderVo.pageSize":pageSize
		   	...
		   	success:function(data){
				...
				if(baseInfo.listType == "doing"){
					url = path+"/workflow/work-flow!toProcessDoingDetail.action";
				}
				if(baseInfo.listType == "wait"){
					url = path+"/workflow/work-flow!toProcessWaitDetail.action";
				}
				if(baseInfo.listType == "done"){
					url = path+"/workflow/work-flow!toProcessDoneDetail.action";
				}
				$.each(pages.result,function(i,n){
					...
					+"<input type='hidden' value='"+n['startManDeptName']+"' name='workOrderVo.startManDeptName'/>"
					+"</form>"
					+""+n['displayPid']+"</td>"
					+"<td>"+n['processName']+"</td>"
