...
jQuery(function(){
	...
	var _limit=20;
	var c=cols.join(",");
	var sql = "select "+c+" from PMRT.TB_MRT_JCDY_JKXSJF_INFO partition(P"+deal_date+") WHERE 1 = 1 ";
			/*if(group_level == 1) {
	...
	$("#search").click(function(){   
		...
		var fsql = getSelect();
		var c=cols.join(",");
		var sql = "select "+c+" from PMRT.TB_MRT_JCDY_JKXSJF_INFO partition(P"+deal_date+") WHERE 1 = 1 "+fsql;
/*		if(group_level == 1) {
		}else if(group_level==2) {
			sql += " AND GROUP_ID_1 = '"+group_id+"' ";
		}else if(group_level == 3) {
