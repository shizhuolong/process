jQuery(function(){
	var cols = ["unit_name","hr_no","user_name","all_sll",
	            "sl_jf","fw_jf","zz_jf","all_sl_cre","all_svr_cre","all_unit_cre","all_unit_money"];
	//excel导出
	...
	var sql = "SELECT T.UNIT_NAME,T.HR_NO,T.USER_NAME,SUM(T.ALL_SLL) AS ALL_SLL,SUM(T.SL_JF) AS SL_JF," +
			"SUM(T.FW_JF) AS FW_JF,SUM(T.ZZ_JF) AS ZZ_JF,SUM(T.ALL_SL_CRE) AS ALL_SL_CRE," +
			"SUM(T.ALL_SVR_CRE) AS ALL_SVR_CRE,SUM(T.ALL_UNIT_CRE) AS ALL_UNIT_CRE,SUM(T.ALL_UNIT_MONEY) AS ALL_UNIT_MONEY " +
			...
			sql += " GROUP BY T.UNIT_NAME, T.HR_NO, T.USER_NAME " +
					"ORDER BY T.UNIT_NAME";
	init(sql);
	function init(sql){ 
		var head="";
		head +="<tr class='attend_th'>" +
				"<th class='attend_th'>营服中心</th>" +
				"<th class='attend_th'>HR编码</th><th class='attend_th'>人员姓名</th>" +
				"<th class='attend_th'>总受理量</th><th class='attend_th'>受理积分</th>" +
				"<th class='attend_th'>服务积分</th><th class='attend_th'>增值业务积分</th>" +
				"<th class='attend_th'>总受理积分</th><th class='attend_th'>服务调节积分</th>" +
				"<th class='attend_th'>区域调节积分</th>" +
	...
	function renderTable(head, data) {
		...
		$.each(data, function(i, strn) {
			...
			for ( var j = 0; j < colNum - 1; j++) {
				if(j > 3 && (strn[j]==null || strn[j]=="0" || strn[j]=="")){
...
function downsAll(){
	...
	var showtext="Sheet";
	   ...
	   var _head=['营服中心','HR编码','人员姓名','总受理量','受理积分','服务积分','增值业务积分','总受理积分',
	              '服务调节积分','区域调节积分','积分金额'];
