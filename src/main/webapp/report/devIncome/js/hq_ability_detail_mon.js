...
function getSql(orgLevel,where){
	...
	if(orgLevel==1){
		return " SELECT '云南省' ROW_NAME,'86000' ROW_ID,'--' UNIT_NAME,'--' HQ_CHAN_CODE,'--' HQ_CHAN_NAME,'--' BUSI_BEGIN_TIME,'--' HQ_STATE,'--' HQ_ZY,'--' THIRD_TYPE,'--' IS_TY,'--' IS_SOCIAL,'--' IS_DS,'--' END_TIME,"+getSumSql(orgLevel)+where;
	}else if(orgLevel==2){
		return " SELECT T1.GROUP_ID_1_NAME ROW_NAME,T1.GROUP_ID_1 ROW_ID,'--' UNIT_NAME,'--' HQ_CHAN_CODE,'--' HQ_CHAN_NAME,'--' BUSI_BEGIN_TIME,'--' HQ_STATE,'--' HQ_ZY,'--' THIRD_TYPE,'--' IS_TY,'--' IS_SOCIAL,'--' IS_DS,'--' END_TIME,"+getSumSql(orgLevel)+where+" GROUP BY T1.GROUP_ID_1,T1.GROUP_ID_1_NAME ORDER BY T1.GROUP_ID_1";
	}else if(orgLevel==3){
