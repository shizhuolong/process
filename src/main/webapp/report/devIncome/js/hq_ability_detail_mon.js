var field=["UNIT_NAME","HQ_CHAN_CODE","HQ_CHAN_NAME","BUSI_BEGIN_TIME","HQ_STATE","HQ_ZY","THIRD_TYPE","FOUR_TYPE","IS_TY","IS_SOCIAL","IS_DS","END_TIME","HQ_TYPE","IS_LS","BUSINESS_ID","BUSINESS_NAME","SCHOOL_ID","SCHOOL_NAME","MAN_COUNT","DEV_2G_NUM","DEV_3G_NUM","DEV_4G_NUM","DEV_KD_NUM","DEV_ZZX_NUM","DEV_NET_NUM","DEV_OTHER_NUM","DEV_COUNT","CHARGE_YW","CHARGE_GW","CHARGE_TOTAL","ACC_NUM","DS_MONEY","DS_SVR_RATE","HQ_ML","HQ_SAL_ML","HQ_CHARGE_SR","HQ_COST_ALL","MAN_COST_CONT","MAN_COST_JM","MAN_COST_FINACE","MAN_COST_ALL","YJ_ALL","HQ_QDBT","TERM_BT","GT_PLACE_RENT","HQ_RENT","HQ_ZX_FEE","KHJR_AMOUNT","KCB_COST","WART_NUM","ADV_FEE","YWYP_FEE","CH_PRO_PRE","SALE_DETAIL_SR","SALE_DETAIL_COST","BG_FEE","CAR_FEE","ZD_FEE","CL_FEE","TX_FEE","COST_RATE","SR_DETAIL_YW","SR_DETAIL_ZX","SR_DETAIL_KD","SR_DETAIL_GH","SR_DETAIL_OTHER","SR_DETAIL_SUM","YF_MON_YW","YF_MON_ZX","YF_MON_KD","YF_MON_GH","YF_MON_OTHER","YF_MON_SUM","YJ_DEV","YJ_ACC","YJ_BOT","YJ_TWO_ALL","COMMFEE_MOB_ZSHB","COMMFEE_ZZX_ZSHB","COMMFEE_KD_ZSHB","COMMFEE_NET_ZSHB","COMMFEE_ZSHB","COMMFEE_DEV_ZSHB","COMMFEE_ACC_ZSHB","COMMFEE_BOT_ZSHB","COMMFEE_ALL_ZSB","SUBS_OWE","SUBS_PAY","SECOND_PAY","COMM_STOP","ORIGINAL_INCOME","YF_COMM"];
var title=[["组织架构","所属基层单元","渠道编码","渠道名称","目前经营者开始合作时间","渠道状态","渠道专业","三级属性","四级属性","是否自建他营","是否社会化合作","是否代收费点(1是 0不是)","清算终止时间","渠道类型","是否连锁","商圈编码","商圈名称","学校编码","学校名称","人数","发展用户数","","","","","","","","出账用户数","","","业务受理量","代收费金额","代收服务费占代收金额比","毛利","其中：零售毛利","出账收入","成本合计","人工成本（应发数）","","","","佣金","渠道补贴","终端补贴","柜台及场地出租收入","房租费","营业厅装修","客户接入成本","卡成本","水电物业安保费","广告宣传费","业务用品及材料费","存货跌价准备","零售收入","零售成本","办公费","车辆使用费","招待费","差旅费","通信费","成本占收比(%)","其中：出账收入明细","","","","","","其中：佣金明细分类一","","","","","","其中：佣金明细分类二","","","","其中：佣金占收比分类一","","","","","其中：佣金占收比分类二","","","","用户欠费","用户预存款","二次续费率","终止清算佣金","原始渠道收入","应付佣金"],
...
function downsAll() {
	...
	var sql = "SELECT T1.GROUP_ID_1_NAME,T1.UNIT_NAME,T1.HQ_CHAN_CODE,T1.HQ_CHAN_NAME,T1.BUSI_BEGIN_TIME,T1.HQ_STATE,T1.HQ_ZY,T1.THIRD_TYPE,T1.FOUR_TYPE,T1.IS_TY,T1.IS_SOCIAL,T1.IS_DS,T1.END_TIME, T1.HQ_TYPE,T1.IS_LS,T1.BUSINESS_ID,T1.BUSINESS_NAME,T1.SCHOOL_ID,T1.SCHOOL_NAME,"+getSumSql(4)+where+" GROUP BY T1.HQ_CHAN_NAME,T1.UNIT_NAME,T1.HQ_CHAN_CODE,T1.HQ_CHAN_NAME,T1.BUSI_BEGIN_TIME,T1.HQ_STATE,T1.HQ_ZY,T1.THIRD_TYPE,T1.IS_TY,T1.IS_SOCIAL,T1.GROUP_ID_1,T1.GROUP_ID_1_NAME,T1.UNIT_ID,T1.IS_DS,T1.END_TIME ,T1.FOUR_TYPE,T1.HQ_TYPE,T1.IS_LS,T1.BUSINESS_ID,T1.BUSINESS_NAME,T1.SCHOOL_ID,T1.SCHOOL_NAME ORDER BY T1.GROUP_ID_1,T1.UNIT_ID,T1.HQ_CHAN_CODE";
...
function getSql(orgLevel,where){
	...
	if(orgLevel==1){
		return " SELECT '云南省' ROW_NAME,'86000' ROW_ID,'--' UNIT_NAME,'--' HQ_CHAN_CODE,'--' HQ_CHAN_NAME,'--' BUSI_BEGIN_TIME,'--' HQ_STATE,'--' HQ_ZY,'--' THIRD_TYPE,'--' FOUR_TYPE,'--' IS_TY,'--' IS_SOCIAL,'--' IS_DS,'--' END_TIME,'--' HQ_TYPE,'--' IS_LS,'--' BUSINESS_ID,'--' BUSINESS_NAME,'--' SCHOOL_ID,'--' SCHOOL_NAME,"+getSumSql(orgLevel)+where ;
	}else if(orgLevel==2){
		return " SELECT T1.GROUP_ID_1_NAME ROW_NAME,T1.GROUP_ID_1 ROW_ID,'--' UNIT_NAME,'--' HQ_CHAN_CODE,'--' HQ_CHAN_NAME,'--' BUSI_BEGIN_TIME,'--' HQ_STATE,'--' HQ_ZY,'--' THIRD_TYPE,'--' FOUR_TYPE,'--' IS_TY,'--' IS_SOCIAL,'--' IS_DS,'--' END_TIME,'--' HQ_TYPE,'--' IS_LS,'--' BUSINESS_ID,'--' BUSINESS_NAME,'--' SCHOOL_ID,'--' SCHOOL_NAME,"+getSumSql(orgLevel)+where+" GROUP BY T1.GROUP_ID_1,T1.GROUP_ID_1_NAME ORDER BY T1.GROUP_ID_1";
	}else if(orgLevel==3){
		return " SELECT T1.UNIT_NAME ROW_NAME,T1.UNIT_ID ROW_ID,'--' UNIT_NAME,'--' HQ_CHAN_CODE,'--' HQ_CHAN_NAME,'--' BUSI_BEGIN_TIME,'--' HQ_STATE,'--' HQ_ZY,'--' THIRD_TYPE,'--' FOUR_TYPE,'--' IS_TY,'--' IS_SOCIAL,'--' IS_DS,'--' END_TIME,'--' HQ_TYPE,'--' IS_LS,'--' BUSINESS_ID,'--' BUSINESS_NAME,'--' SCHOOL_ID,'--' SCHOOL_NAME,"+getSumSql(orgLevel)+where+" GROUP BY T1.GROUP_ID_1,T1.GROUP_ID_1_NAME,T1.UNIT_ID,T1.UNIT_NAME ORDER BY T1.GROUP_ID_1,T1.UNIT_ID";
	}else if(orgLevel==4){
		return " SELECT T1.HQ_CHAN_NAME ROW_NAME,T1.UNIT_NAME,T1.HQ_CHAN_CODE,T1.HQ_CHAN_NAME,T1.BUSI_BEGIN_TIME,T1.HQ_STATE,T1.HQ_ZY,T1.THIRD_TYPE,T1.FOUR_TYPE,T1.IS_TY,T1.IS_SOCIAL,T1.IS_DS,T1.END_TIME, T1.HQ_TYPE,T1.IS_LS,T1.BUSINESS_ID,T1.BUSINESS_NAME,T1.SCHOOL_ID,T1.SCHOOL_NAME,"+getSumSql(orgLevel)+where+" GROUP BY T1.HQ_CHAN_NAME,T1.UNIT_NAME,T1.HQ_CHAN_CODE,T1.HQ_CHAN_NAME,T1.BUSI_BEGIN_TIME,T1.HQ_STATE,T1.HQ_ZY,T1.THIRD_TYPE,T1.FOUR_TYPE,T1.IS_TY,T1.IS_SOCIAL,T1.GROUP_ID_1,T1.GROUP_ID_1_NAME,T1.UNIT_ID,T1.IS_DS,T1.END_TIME ,T1.HQ_TYPE,T1.IS_LS,T1.BUSINESS_ID,T1.BUSINESS_NAME,T1.SCHOOL_ID,T1.SCHOOL_NAME ORDER BY T1.GROUP_ID_1,T1.UNIT_ID,T1.HQ_CHAN_CODE";
	}
  }
