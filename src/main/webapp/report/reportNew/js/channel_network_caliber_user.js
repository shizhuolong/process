...
$(function(){
	...
	report=new LchReport({
		...
		getSubRowsCallBack:function($tr){
			...
			//条件
			if(isAll==1){
				where+= " FROM PODS.VIEW_ODS_WLKJ_CZYH_MON_ZX T1, "+
				" PCDE.TB_CDE_REGION_CODE T2 "+
				" WHERE DEAL_DATE= "+dealDate+" and T1.GROUP_ID_1=T2.GROUP_ID_1(+)";
			}else if(isAll==2){
				where+= " FROM PODS.VIEW_ODS_WLKJ_CZYH_TMP_ZL_ZX T1, "+
				" PCDE.TB_CDE_REGION_CODE T2 "+
				" WHERE DEAL_DATE= "+dealDate+" and T1.GROUP_ID_1=T2.GROUP_ID_1(+)";
			}else{
				where+= " FROM PODS.VIEW_ODS_WLKJ_CZYH_TMP_CL_ZX T1, "+
				" PCDE.TB_CDE_REGION_CODE T2 "+
				" WHERE DEAL_DATE= "+dealDate+" and T1.GROUP_ID_1=T2.GROUP_ID_1(+)";
			}
			if(regionCode!=''){
