...
function listRegions(){
	var sql = "SELECT DISTINCT T.GROUP_ID_1,T.GROUP_ID_1_NAME FROM PMRT.TAB_MRT_BROAD_MANAGE_MON PARTITION(P"+qdate+") T WHERE 1=1 ";
	//权限
	var orgLevel=$("#orgLevel").val();
	var code=$("#code").val();
	if(orgLevel==1){
...
function listUnits(regionCode){
	var $unit=$("#unitId");
	var sql = "SELECT DISTINCT T.UNIT_ID,T.UNIT_NAME FROM PMRT.TAB_MRT_BROAD_MANAGE_MON PARTITION(P"+qdate+") T WHERE 1=1 AND UNIT_ID IS NOT NULL";
	if(regionCode!=''){
