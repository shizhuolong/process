		...
		success:function(data){
   		...
   		$.each(pages.rows,function(i,n){
			...
			+"<td style='display:none;'>"+userid+"</td>"
   			...
   			+"<td><a href='#' onclick='create(this,event)' chl_name='"+n['HQ_CHAN_NAME']+"' chl_code='"+n['HQ_CHAN_CODE']+"' p_levl='"+orgLevel+"' p_code='"+p_code+"' name='"+name+"' userid='"+userid+"' username='"+username+"' phone='"+phone+"'  chanl_type='"+n['CHLTYPE']+"'>选择</a></td>"
			content+="</tr>";
		});
		if(content != "") {
